...
public class BinarySearch02 {
    ...
    private int count(int[] input, int capacity) {
        int count = 1;
        int sum = 0;
        for (int i : input) {
            if (sum + i > capacity) {
    ...
    private int solution(int target, int[] input) {
        int answer = 0;
        int lt = Arrays.stream(input).max().getAsInt(); // 1곡당 1DVD 할때 dvd 최소 용량
        int rt = Arrays.stream(input).sum(); // dvd 1개에 다 담을 수 있는 용량
        int mid = (lt + rt) / 2; // target 개에 담으려할때 최소 용량

        while (lt <= rt) {
            int count = count(input, mid);
            if (count <= target) {
                answer = mid;
                rt = mid - 1;
            } else {
    ...
    public static void main(String[] args) {
        ...
        int result = bs.solution(3, new int[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
